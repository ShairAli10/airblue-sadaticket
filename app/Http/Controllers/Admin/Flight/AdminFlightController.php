...
class AdminFlightController extends Controller
{
    ...
    public function searchAvailability(Request $request){
        ...
        $provider = $helper->useTrait($request->api);
        $lowFareSearchResp = [];
        if ($provider && method_exists($provider, 'lowFareSearch')) {
            $lowFareSearchResp = call_user_func_array([$provider, 'lowFareSearch'], [$requestData]);
            if($lowFareSearchResp['status']  == 200){
                $response = $lowFareSearchResp['msg'];
                ...
                return json_encode(['message' => $lowFareSearchResp['msg']]);
            }
        } else {
            return response()->json([
                'status' => 'error',
                'message' => 'Invalid provider or method.'
            ], 400);
        }

        if($lowFareSearchResp['status'] == 200){
            $response = $lowFareSearchResp;
            $results = array(
                'status' => 200,
                'msg' => $prevRes,
            );
