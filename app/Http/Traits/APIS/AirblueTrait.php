...
trait AirblueTrait
{
    ...
    public static function lowFareSearch($request)
    {
        ...
        // for live server
        // $certPath = base_path('certs/combined.pem');
    ...
    public static function makeResponseReturn($response, $request)
    {
        ...
        if (! empty($responseData)) {
            ...
            foreach ($responseData as $key => $flight) {
                ...
                $taxes_1  = (isset($flight['AirItineraryPricingInfo']['ItinTotalFare']['Taxes'])
                    ? (int) $flight['AirItineraryPricingInfo']['ItinTotalFare']['Taxes']['@attributes']['Amount']
                    : 0)
                     + (isset($flight['AirItineraryPricingInfo']['ItinTotalFare']['Fees'])
                    ? (int) $flight['AirItineraryPricingInfo']['ItinTotalFare']['Fees']['@attributes']['Amount']
                    : 0);
            ...
            for ($i = 0; $i < $maxCount; $i++) {
                ...
                $totalFare        = 0;
                $total_base_fare  = ['ADT' => 0, 'CHD' => 0, 'INF' => 0];
                $total_taxes_Fare = ['ADT' => 0, 'CHD' => 0, 'INF' => 0];
                ...
                if (isset($goingFlights[$i])) {
                    ...
                    foreach ($goingFlights[$i]['Fares'][0]['PassengerFares'] as $passengerFare) {
                        $paxType = $passengerFare['PaxType'];
                        $total_base_fare[$paxType] += $passengerFare['BasePrice'];
                        $total_taxes_Fare[$paxType] += $passengerFare['Taxes'];
                    }
                }
                ...
                if (isset($returnFlights[$i])) {
                    ...
                    foreach ($returnFlights[$i]['Fares'][0]['PassengerFares'] as $passengerFare) {
                        $paxType = $passengerFare['PaxType'];
                        $total_base_fare[$paxType] += $passengerFare['BasePrice'];
                        $total_taxes_Fare[$paxType] += $passengerFare['Taxes'];
                    }
                ...
                if (! empty($flightPair['Flights'])) {
                    ...
                    foreach ($flightPair['Flights'][0]['Fares'][0]['PassengerFares'] as &$passengerFare) {
                        $paxType                     = $passengerFare['PaxType'];
                        $passengerFare['BasePrice']  = $total_base_fare[$paxType];
                        $passengerFare['Taxes']      = $total_taxes_Fare[$paxType];
                        $passengerFare['TotalPrice'] = $total_base_fare[$paxType] + $total_taxes_Fare[$paxType];
                    }
                    ...
                    if (isset($flightPair['Flights'][1])) {
                        ...
                        foreach ($flightPair['Flights'][1]['Fares'][0]['PassengerFares'] as &$passengerFare) {
                            $paxType                     = $passengerFare['PaxType'];
                            $passengerFare['BasePrice']  = $total_base_fare[$paxType];
                            $passengerFare['Taxes']      = $total_taxes_Fare[$paxType];
                            $passengerFare['TotalPrice'] = $total_base_fare[$paxType] + $total_taxes_Fare[$paxType];
