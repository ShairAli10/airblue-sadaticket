        ...
        <section class="flight-results">
            <div class="row g-2 gy-3">
                ...
                <div class="col-lg-8">
                    ...
                    <section class="card px-3 d-block mb-2" id="date_swiper"></section>
                    <section class="card px-3 d-block" id="airlineSlider"></section>
...
<script>
    $(document).ready(function() {
        ...
        const defaultDate = flatpickr.formatDate(futureDate, "d-m-Y");
                        ...
                        for (const api of apiArray) {
                            try {
                                ...
                                if (obj.message == 'success') {
                                    ...
                                    applySorting('Price (Lowest)')
                                } else {
                                    if(obj.message != 'No Flights available on this date, Please try another one..'){
                                        Swal.fire({
                                            text: obj.message,
                                            icon: "warning",
                                            buttonsStyling: false,
                                ...
                                var message = data.errors.permissionArray ? data.errors.permissionArray[0] : data.errors.roleName[0];
                                Swal.fire({
                                    text: message,
                                    icon: "error",
                                    buttonsStyling: false,
...
<script>
    ...
    // /////////////////////////////Typing origin///////////////////////////////////
    $(document).ready(function () {
        const searchairport = async (searchBox,airportList) => {
            ...
            outputHtml(fits,airportList);
        };
        const outputHtml = (fits,airportList) => {
            if (fits.length > 0) {
                const airportFits = fits
                .map(
                    (fit) =>
                    `<div class="list-item my-1 position-relative d-flex justify-content-between">
                        <div class="item rounded-1 p-2 d-flex align-items-center w-100">
                            <div class="detail text-truncate" title="${fit.name} (${fit.iata_code})">
                                <h6 class="fw-semibold text-truncate text-dark mb-0">${fit.city}</h6>
                                <p class="small text-gray-300 mb-0">${fit.country}</p>
                            </div>
                            <span class="ms-2 small airport-code ms-auto align-self-start">${fit.iata_code}</span>
                        </div>
                    </div>`
                )
                .join('');
                $(airportList).html(airportFits);
            }
        };
        
        function handleInput(inputName, menuId) {
            var searchInput = $('#' + inputName);
            var airportList = $('#' + menuId);

            searchInput.on('input', function () {
                searchairport($(this).val(),airportList);
            });
        }
    ...
    $(document).ready(function () {
        ...
        function populateDropdown(menuId, airports, inputValue) {
            ...
            var typedInputResults = airports.filter(function (airport) {
                return airport.iata_code.toLowerCase().startsWith(inputValue.toLowerCase()) ||
        ...
        function appendAirportItem(dropdownMenu, airport) {
            dropdownMenu.append(`<div class="list-item my-1 position-relative d-flex justify-content-between">
                                    <div class="item rounded-1 p-2 d-flex align-items-center w-100">
                                        <div class="detail text-truncate" title="${airport.city}">
                                            <h6 class="fw-semibold text-truncate text-dark mb-0">${airport.city}</h6>
                                            <p class="small text-gray-300 mb-0">${airport.name}</p>
                                        </div>
                                        <span class="ms-2 small airport-code ms-auto align-self-start">${airport.iata_code}</span>
                                    </div>
                                </div>`);
        }
    });
