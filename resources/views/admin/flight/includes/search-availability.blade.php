@if ($results['status'] == '200')
    @foreach ($results['msg'] as $flight)
        ...
        <a data-bs-toggle="offcanvas" href="#flightSidepanel" aria-controls="flightSidepanel"
            class="flight-card card text-reset" data-price="{{ $totalPrice }}" data-airline="{{ $airCode }}"
            data-api="{{ $flight['api'] }}" data-stops="{{ $stopss }}" data-ref-key="{{ $flight['itn_ref_key'] }}"
            data-fare-type="{{ $FareType }}" data-duration="{{ $TotalDuration }}">
            <div class="card-body">
                        ...
                        <div class="col-md-12">
                            <div class="row g-3 gy-4 gy-md-3">
                                <div class="col-lg-6 col-md-6 col-12 order-md-2">
                                    <div class="row g-2 align-items-center">
                                        ...
                                        <div
                                            ...
                                            <div
                                                class="route-line-wrapper w-100 d-flex justify-content-between align-items-center">
                                            ...
                                            <p class="small mb-0">
                                                ...
                                                @if ($airNum['MarketingAirline']['Code'] != $airNum['OperatingAirline']['Code'])
                                                    <span class="text-primary fw-bold">
                                                        ({{ $airNum['OperatingAirline']['Code'] }}-{{ $airNum['OperatingAirline']['FlightNumber'] }})</span>
                                                @endif
                                            </p>
                                        @endforeach
                                    </div>
                                ...
                                <div class="col-lg-3 col-md-3 col-4 text-end order-md-3">
                                    @if (strpos($CabinClass, '(') !== false && strpos($CabinClass, ')') !== false)
                                        <h6 class="mb-0">{{ $CabinClass }}</h6>
                                    @else
            ...
            <div class="card-footer d-flex align-items-center justify-content-between py-2">
                ...
                <div class="price text-end">
                    ...
                    <h4 class="fw-semibold text-secondary mb-0 fs-50" style="display: inline;">{{ $currency }}
                        {{ number_format((int) $totalPrice) }}</h4>
                </div>
